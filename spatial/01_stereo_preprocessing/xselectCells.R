...
#source("app.R")
xSelectCells <- function(input_seurat_obj, type = "GEM", img = "image",feature = "active.ident") {
  my_barcodes  <- c()
  library(shiny)
  ...
  server_1 <- function(input, output, session) {
    ...
    sc_seurat_umap_inter_base <- reactive({
      ...
      #ident = as.data.frame(sc$marker1)
      ident = as.data.frame(sc@meta.data[,feature])
      ...
      if(type == "spatial" || type == "spacial"){
        ...
        embeds = as.data.frame(Seurat::Embeddings(sc[["cod"]]),col.names = T)
    ...
    umap_for_brush <- reactive({
      ...
      library(dplyr)
      p = plotly::plot_ly(sc,type="scatter", mode = "markers",
                      x = ~X, y = ~Y,
                      key = ~keys,
                      color = ~ident,
